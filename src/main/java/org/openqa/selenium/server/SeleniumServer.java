...
public class SeleniumServer implements SslCertificateGenerator {
	...
	public static void main(String[] args) throws Exception {
		...
		System.setProperty(
				"org.openqa.jetty.http.HttpRequest.maxFormContentSize", "0"); // default
																				// max
																				// is
																				// 200k;
																				// zero
	...
	public static synchronized Log configureLogging(LoggingOptions options,
		...
		if (options.dontTouchLogging()) {
			return LogFactory
					.getLog("org.openqa.selenium.server.SeleniumServer");
		}

		LoggingManager.configureLogging(options, debugMode);

		seleniumServerJettyLogger = LogFactory
				.getLog("org.openqa.selenium.server.SeleniumServer");
		if (null != options.getLogOutFile()) {
			seleniumServerJettyLogger.info("Writing debug logs to "
					+ options.getLogOutFile());
		}
	...
	private void logVersionNumber() throws IOException {
		...
		InputStream stream = ClassPathResource
				.getSeleniumResourceAsStream("/VERSION.txt");
		if (stream == null) {
		...
		LOGGER.info(String.format(
				"v%s%s, with Core v%s%s. Built from revision %s", rcVersion,
				rcRevision, coreVersion, coreRevision, info.getBuildRevision()));
	}
	...
	private void assembleHandlers(boolean slowResources,
		...
		// Both the selenium and webdriver contexts must be able to share
		// sessions
	...
	private HttpContext createWebDriverRemoteContext(
		...
		webdriverContext.setInitParameter("webdriver.server.session.timeout",
				String.valueOf(sessionTimeout));
		webdriverContext.setInitParameter("webdriver.server.browser.timeout",
				String.valueOf(browserTimeout));
		webdriverContext.setAttribute(DriverServlet.SESSIONS_KEY,
				webDriverSessions);
		webdriverContext.setContextPath("/wd");
		ServletHandler handler = new ServletHandler();
		handler.addServlet("WebDriver remote server", "/hub/*",
				DriverServlet.class.getName());
		webdriverContext.addHandler(handler);

		LOGGER.info(format(
				"RemoteWebDriver instances should connect to: http://%s:%d/wd/hub",
				networkUtils.getPrivateLocalAddress(), getPort())); // todo:
																	// This is
																	// still
																	// buggy
																	// because
	...
	private void addSecurityHandler(HttpContext context) {
		...
		context.addSecurityConstraint(
				"/org/openqa/selenium/tests/html/basicAuth/*", constraint);
	...
	protected ProxyHandler makeProxyHandler(
		...
		return new ProxyHandler(configuration.trustAllSSLCertificates(),
				...
				configuration.getPort(), shutdownLock);
	}
	...
	public void start() throws Exception {
		System.setProperty(
				"org.openqa.jetty.http.HttpRequest.maxFormContentSize", "0"); // default
																				...
																				// infinite
		int myFailedPorts = 0;
		while (myFailedPorts++ < 100) {
			try {
				server.start();
				myFailedPorts += 100;
			} catch (MultiException e) {
				int myPort = getPort();
				String myBindExceptionString = "Selenium is already running on port "
						+ myPort + ". Or some other service is.";
				LOGGER.info(myBindExceptionString);
				if (myFailedPorts >= 99) {
					if (e.getExceptions().size() == 1
							&& e.getException(0) instanceof BindException) {
						throw new BindException(myBindExceptionString);
					}
					throw e;
				} else {
					configuration.setPort(myPort+1);
				}
			}
		}

		shutDownHook = new Thread(new ShutDownHook(this)); // Thread safety
															// reviewed
	...
	public void stop() {
		...
		while (numTries <= MAX_SHUTDOWN_RETRIES) {
			...
			try {
				...
				break;
			} catch (Exception ex) { // org.openqa.jetty.jetty.Server.stop()
										// throws Exception
				LOGGER.error(ex);
	...
	protected void runHtmlSuite() {
		...
		try {
			...
			if ((resultDir != null) && !resultDir.exists()
					&& !resultDir.mkdirs()) {
				RemoteControlLauncher
						.usage("can't create directory for result file "
								+ resultFilePath);
				System.exit(1);
			}
